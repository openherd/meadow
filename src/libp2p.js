...
export const setupLibp2pNode = async () => {
  ...
  function discover() {
    config.relayServers.map(async server => {
      try {
        (await (await fetch(`${server}/_openherd/outbox?random=${Math.random().toString(32).slice(2)}`)).json()).forEach(async post => {
          await importPost({ ...post, raw: JSON.stringify(post) })
        })
      ...
      try {
        ((await fetch(`${server}/_openherd/inbox?random=${Math.random().toString(32).slice(2)}`, {
          body: (await db.posts.getMany()).map(a => JSON.parse(a.raw)),
          method: "POST"
        })).json())
